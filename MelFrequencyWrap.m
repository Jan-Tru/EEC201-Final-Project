...
function MelWrapArray = MelFrequencyWrap(melPointAmount, FFTArray, SampleRate)
    lowFreqMel = 0;
    ...
    % Preallocate MelWrapArray
    MelWrapArray = zeros(length(fftPoints)-2, size(FFTArray,2)); % -2 to exclude endpoints
    
    % For each FFTArray, l, from 1 to the last fft column
    for l = 1:size(FFTArray,2)
